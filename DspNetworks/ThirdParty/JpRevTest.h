...
#include "src/JpRevTest.cpp"
#if (FAUST_INPUTS - FAUST_OUTPUTS) > 0
#error Number of inputs and outputs in faust code must match!
#endif
namespace project {
struct JpRevTestMetaData {
		SN_NODE_ID("JpRevTest");
};
template <int NV, class ModParameterClass=scriptnode::parameter::empty_list>
using JpRevTest = scriptnode::faust::faust_static_wrapper<NV, ModParameterClass, _JpRevTest, JpRevTestMetaData, FAUST_OUTPUTS>;
} // namespace project
 // undef dummy macros
