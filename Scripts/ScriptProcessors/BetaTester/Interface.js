...
//const var FaustTest = Synth.getEffect("FaustTest");
const var VERBERV = Synth.getEffect("VERBERV");
...
//KNOB
 ...
 laf.registerFunction("drawRotarySlider", function(g, obj)
 {
 		...
 		if(obj.text.indexOf("AWARENESS") != -1)
 		{
	 		g.setColour(obj.bgColour);
	 		if(btnIntelliOnOff.getValue() == 1)
	 		{
		 		g.setOpacity(1.0);
...
const var btnPreOnOff = Content.getComponent("btnPreOnOff");

btnPreOnOff.setControlCallback(predelayPower);

inline function predelayPower(component, value)
{
	if (value == 1)
	{
		VERBERV.setAttribute(VERBERV.TempoSync, 1);
	}
	else
	{
		VERBERV.setAttribute(VERBERV.TempoSync, 0);
	}
}
...
{	
	...
	switch(tempoSignature) { //Switch cases base on tempoSignature's value
		case 0: 
			...
			VERBERV.setAttribute(VERBERV.BeatTime, 0);
			break;
		
		case 1:
			lbPredelayTempo.set("text" , "1/2");
			VERBERV.setAttribute(VERBERV.BeatTime, 2);
			break;
		
		case 2:
			lbPredelayTempo.set("text" , "1/4");
			VERBERV.setAttribute(VERBERV.BeatTime, 5);
			break;
		case 3:
			lbPredelayTempo.set("text" , "1/8");
			VERBERV.setAttribute(VERBERV.BeatTime, 8);
			break;
		
		case 4:
			lbPredelayTempo.set("text" , "1/16");
			VERBERV.setAttribute(VERBERV.BeatTime, 11);
			break;
	}
...
inline function onKnob2Control(component, value)
{
	...
	//FaustTest.setAttribute(FaustTest.Blend, value);
	VERBERV.setAttribute(VERBERV.Blend, value);
};
...
inline function onKnob3Control(component, value)
{
	...
	//FaustTest.setAttribute(FaustTest.Space, 1 - value);
	//FaustTest.setAttribute(FaustTest.Feedback,  value);
	VERBERV.setAttribute(VERBERV.Space, 1 - value);
	VERBERV.setAttribute(VERBERV.Feedback, value);
...
levTimer.setTimerCallback(function(){
	...
	inline function onCableChange(value)
	{
		...
		if (btnIntelliOnOff.getValue() == 1)
		{
			//FaustTest.setAttribute(FaustTest.VerbGate, 1 - (bufferValue * Knob1.getValue()));
			VERBERV.setAttribute(VERBERV.VerbGate, 1 - (bufferValue * Knob1.getValue()));
			intelliVerb.setValue((bufferValue * Knob1.getValue()) * 7);
							
		}
		else
		{
			VERBERV.setAttribute(VERBERV.VerbGate, 1);
			knbVerb.setValue(bufferValue * 200);			
		}
	}
